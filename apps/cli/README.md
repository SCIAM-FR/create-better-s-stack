...
![demo](https://cdn.jsdelivr.net/gh/amanvarshney01/create-better-s-stack@master/demo.gif)
...
```bash
Usage: create-better-s-stack [project-directory] [options]
...
create-better-s-stack create-json --input '{"projectName":"my-app","yes":true,"dryRun":true}'
create-better-s-stack add-json --input '{"projectDir":"./my-app","addons":["wxt"],"addonOptions":{"wxt":{"template":"react"}}}'
create-better-s-stack create-json --input '{"projectName":"db-app","database":"postgres","orm":"drizzle","dbSetup":"neon","dbSetupOptions":{"mode":"manual"}}'
...
create-better-s-stack schema --name dbSetupOptions
create-better-s-stack schema --name cli

# Local stdio MCP server
npx @sciam-fr/create-better-s-stack@latest mcp
```

To install Better S Stack into supported agent configs with `add-mcp` and avoid relying on a global CLI install:

```bash
npx -y add-mcp@latest "npx -y @sciam-fr/create-better-s-stack@latest mcp"
```

When you scaffold with the `mcp` addon, Better S Stack itself can also be installed into supported agent configs through `add-mcp` using a package runner command instead of assuming a global CLI install. For Bun projects, the generated config uses the equivalent `bunx @sciam-fr/create-better-s-stack@latest mcp` server command inside `add-mcp`.
...
BTS_TELEMETRY_DISABLED=1 npx @sciam-fr/create-better-s-stack
...
```bash
npx @sciam-fr/create-better-s-stack --yes
```

Validate a command without writing files:

```bash
npx @sciam-fr/create-better-s-stack --yes --dry-run
```

Create a project with specific options:

```bash
npx @sciam-fr/create-better-s-stack --database postgres --orm drizzle --auth better-auth --addons pwa biome
```

Create a project with Elysia backend and Node.js runtime:

```bash
npx @sciam-fr/create-better-s-stack --backend elysia --runtime node
```

Create a project with multiple frontend options (one web + one native):

```bash
npx @sciam-fr/create-better-s-stack --frontend tanstack-router native-bare
```

Create a project with examples:

```bash
npx @sciam-fr/create-better-s-stack --examples todo ai
```

Create a project with Turso database setup:

```bash
npx @sciam-fr/create-better-s-stack --database sqlite --orm drizzle --db-setup turso
```

Create a project with Supabase PostgreSQL setup:

```bash
npx @sciam-fr/create-better-s-stack --database postgres --orm drizzle --db-setup supabase --auth better-auth
```

Create a project with Convex backend:

```bash
npx @sciam-fr/create-better-s-stack --backend convex --frontend tanstack-router
```

Create a project with documentation site:

```bash
npx @sciam-fr/create-better-s-stack --addons starlight
```

Create a minimal TypeScript project with no backend:

```bash
npx @sciam-fr/create-better-s-stack --backend none --frontend tanstack-router
```

Create a backend-only project with no frontend:

```bash
npx @sciam-fr/create-better-s-stack --frontend none --backend hono --database postgres --orm drizzle
```

Create a simple frontend-only project:

```bash
npx @sciam-fr/create-better-s-stack --backend none --frontend next --addons none --examples none
```

Create a Cloudflare Workers project:

```bash
npx @sciam-fr/create-better-s-stack --backend hono --runtime workers --database sqlite --orm drizzle --db-setup d1
```

Create a self-hosted fullstack project on Cloudflare with D1:

```bash
npx @sciam-fr/create-better-s-stack --backend self --frontend next --api trpc --database sqlite --orm drizzle --db-setup d1 --web-deploy cloudflare
```

Create a minimal API-only project:

```bash
npx @sciam-fr/create-better-s-stack --frontend none --backend hono --api trpc --database none --addons none
```
