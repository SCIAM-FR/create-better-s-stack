    ...
    .input(
      z.tuple([
        ...
        z.object({
          ...
          ecosystem: EcosystemSchema.optional().describe("Project ecosystem (ts or python)"),
          pythonApp: PythonAppSchema.optional().describe("Python application shape"),
          pythonOrm: PythonOrmSchema.optional().describe("Python ORM"),
          pythonMl: z.array(PythonMlSchema).optional().describe("Python ML capability packs"),
          pythonGenai: PythonGenaiListSchema.optional().describe("Python GenAI capability packs"),
          pythonAgents: z
            .array(PythonAgentsSchema)
            .optional()
            .describe("Python agents capability packs"),
          accelerator: AcceleratorSchema.optional().describe("Compute accelerator for torch packs"),
          pythonStarter: z.boolean().optional().describe("Emit opt-in starter modules per pack"),
...
export async function createVirtual(
  ...
  const isPython = (options.ecosystem || "ts") === "python";

  const config: ProjectConfig = {
    projectName: options.projectName || "my-project",
    projectDir: "/virtual",
    relativePath: "./virtual",
    ecosystem: options.ecosystem || "ts",
    pythonApp: options.pythonApp || "none",
    pythonOrm: options.pythonOrm || "none",
    pythonMl: options.pythonMl || [],
    pythonGenai: options.pythonGenai || [],
    pythonAgents: options.pythonAgents || [],
    accelerator: options.accelerator || "cpu",
    pythonStarter: options.pythonStarter ?? false,
    addonOptions: options.addonOptions,
    dbSetupOptions: options.dbSetupOptions,
    database: options.database || "none",
    orm: options.orm || "none",
    backend: options.backend || (isPython ? "none" : "hono"),
    runtime: options.runtime || (isPython ? "none" : "bun"),
    frontend: options.frontend || (isPython ? [] : ["tanstack-router"]),
    addons: options.addons || [],
    examples: options.examples || [],
    auth: options.auth || "none",
    payments: options.payments || "none",
    git: options.git ?? false,
    packageManager: options.packageManager || (isPython ? "uv" : "bun"),
    install: false,
    dbSetup: options.dbSetup || "none",
    api: options.api || (isPython ? "none" : "trpc"),
