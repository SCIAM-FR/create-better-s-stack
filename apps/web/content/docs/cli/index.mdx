...
The Better-S-Stack CLI provides several commands to manage your TypeScript projects.

## `create` (Default Command)

Creates a new Better-S-Stack project.

```bash
create-better-s-stack [project-directory] [options]
```
...
create-better-s-stack --database postgres --backend hono --frontend tanstack-router
...
Adds addons to an existing Better-S-Stack project.

```bash
create-better-s-stack add [options]
```
...
create-better-s-stack add

# Add specific addons
create-better-s-stack add --addons pwa tauri --install

# Add addons in a specific project directory
create-better-s-stack add --project-dir ./my-app --addons mcp skills
```
...
Displays Better-S-Stack sponsors.

```bash
create-better-s-stack sponsors
```
...
Opens the Better-S-Stack documentation in your default browser.

```bash
create-better-s-stack docs
```

Opens `https://better-s-stack.sciam.fr/docs` in your browser.
...
```bash
create-better-s-stack builder
```

Opens `https://better-s-stack.sciam.fr/new` where you can configure your stack visually.
...
```bash
create-better-s-stack history [options]
```
...
```bash
create-better-s-stack api-server \
  --frontend none \
  --backend hono \
  --database postgres \
  --orm drizzle \
...
create-better-s-stack add --addons tauri starlight --install
```

### Show Local History

```bash
create-better-s-stack history --limit 20
```
...
```typescript
import { create } from "@sciam-fr/create-better-s-stack";
