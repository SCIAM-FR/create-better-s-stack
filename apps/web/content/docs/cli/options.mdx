...
```bash
create-better-s-stack --template t3
```
...
```bash
create-better-s-stack --manual-db
```
...
```bash
create-better-s-stack my-app --yes --dry-run
```
...
```bash
create-better-s-stack --package-manager bun
```
...
```bash
create-better-s-stack --no-install
```
...
create-better-s-stack --no-render-title
```
...
create-better-s-stack my-app --yes --directory-conflict overwrite

# Safely create a new directory name if it exists
create-better-s-stack my-app --yes --directory-conflict increment
```
...
create-better-s-stack --disable-analytics

# Enable analytics collection (default)
create-better-s-stack --no-disable-analytics
```

Analytics help improve Better-S-Stack by providing insights into usage patterns. When disabled, no data is collected or transmitted.
...
```bash
create-better-s-stack --database postgres
```
...
```bash
create-better-s-stack --database postgres --orm drizzle
```
...
```bash
create-better-s-stack --database postgres --db-setup neon
```
...
```bash
create-better-s-stack --backend hono
```
...
```bash
create-better-s-stack --backend hono --runtime bun
```
...
```bash
create-better-s-stack --api trpc
```
...
```bash
# Single web frontend
create-better-s-stack --frontend tanstack-router

# Web + native frontend
create-better-s-stack --frontend next native-uniwind

# Backend-only
create-better-s-stack --frontend none
```
...
```bash
create-better-s-stack --auth better-auth
create-better-s-stack --auth clerk
create-better-s-stack --auth none
```
...
```bash
create-better-s-stack --payments polar --auth better-auth
```
...
- `mcp`: Install MCP servers, including Better S Stack itself, with add-mcp
...
```bash
create-better-s-stack --addons pwa biome husky
```
...
```bash
create-better-s-stack --examples todo ai
```
...
```bash
create-better-s-stack --web-deploy cloudflare
```
...
```bash
create-better-s-stack --server-deploy cloudflare
```
...
create-better-s-stack history

# Show last 5 projects
create-better-s-stack history --limit 5

# Output as JSON
create-better-s-stack history --json

# Clear all history
create-better-s-stack history --clear
```
