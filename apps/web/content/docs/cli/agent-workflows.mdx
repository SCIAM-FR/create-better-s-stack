...
Better-S-Stack now supports a fully JSON-first workflow for agents and automation:
...
```bash
create-better-s-stack create-json --input '{
  "projectName": "my-app",
...
```bash
create-better-s-stack add-json --input '{
  "projectDir": "./my-app",
...
Run Better-S-Stack itself as a local MCP server over stdio:
...
```bash
npx -y add-mcp@latest "npx -y create-better-s-stack@latest mcp"
```
...
This is the best path when an MCP-capable client should scaffold or modify Better-S-Stack projects without shelling out to a prompt-driven CLI flow.
...
3. Resolve the top-level `ecosystem` first (`ts` or `python`), then build a full explicit stack config.
...
- `ecosystem` is the top-level workflow discriminator. Use `ts` for TypeScript stacks and `python` for Python apps.
...
If you scaffold a project with the `mcp` addon, Better-S-Stack itself is now one of the recommended MCP servers. The addon installs it through `add-mcp` with a package runner command so the generated config does not rely on a globally installed CLI:

```bash
npx -y add-mcp@latest "npx -y create-better-s-stack@latest mcp"
```

For Bun projects, the generated config uses the equivalent `bunx create-better-s-stack@latest mcp` server command inside `add-mcp`. That means MCP-capable agents inside the generated project can talk back to Better-S-Stack without any extra global install, alongside other recommended docs, framework, and database MCP servers.
...
When `skills` is used with the `evlog` addon, Better-S-Stack recommends Evlog's agent skills from `https://www.evlog.dev`: `review-logging-patterns` and `analyze-logs`.
...
```bash
create-better-s-stack --yes --dry-run
create-better-s-stack create-json --input '{"projectName":"my-app","yes":true,"dryRun":true}'
create-better-s-stack add-json --input '{"projectDir":"./my-app","addons":["mcp"],"dryRun":true}'
```
...
Better-S-Stack persists structured configuration in `bts.jsonc`, including:
