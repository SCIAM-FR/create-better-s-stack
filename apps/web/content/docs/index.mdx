---
title: Quick Start
description: Create your first Better-S-Stack project in minutes
---
...
Before running it, authenticate once: create a [GitHub personal access token](https://github.com/settings/tokens)
...
Export the token so npm/bun/pnpm can read it (`${GITHUB_TOKEN}` is expanded at
...
```npm
npm create @sciam-fr/better-s-stack@latest
```
...
```npm
npm create @sciam-fr/better-s-stack@latest --yes
```
...
```npm
npm create @sciam-fr/better-s-stack@latest builder
```
